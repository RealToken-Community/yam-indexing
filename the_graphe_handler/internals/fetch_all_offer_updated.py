...
def fetch_all_offer_updated(api_key: str, url: str, session: requests.Session = None) -> List[Dict[str, Any]]:
    """
    Fetch all offerUpdated entities from The Graph subgraph with deterministic pagination.
    ...
    query_template = """
    query GetOfferUpdated($first: Int!, $lastId: String!, $block: Int!) {
      offerUpdateds(
        first: $first,
        where: { id_gt: $lastId },
        orderBy: id,
