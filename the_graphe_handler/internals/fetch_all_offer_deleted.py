...
def fetch_all_offer_deleted(api_key: str, url: str, session: requests.Session = None) -> List[Dict[str, Any]]:
    """
    Fetch all offerDeleted entities from The Graph subgraph with deterministic pagination.
    ...
    query_template = """
    query GetOfferDeleted($first: Int!, $lastId: String!, $block: Int!) {
      offerDeleteds(
        first: $first,
        where: { id_gt: $lastId },
        orderBy: id,
